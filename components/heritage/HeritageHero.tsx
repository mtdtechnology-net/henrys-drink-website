...
export function HeritageHero({ data }: HeritageHeroProps) {
  const sectionData = data?.heroSection;

  const titleText = sectionData?.HeroTitle || "WHERE THE STORY BEGINS";
  const words = titleText.split(" ");

  const descriptionText =
    sectionData?.HeroDescription ||
    "From the vineyards of Bordeaux to generations of tradition, Henry's is rooted in heritage, craftsmanship, and the art of taking time.";
    ...
    >
      ...
      <div className="relative z-[3] mx-auto flex min-h-screen min-h-[100svh] w-full max-w-[1800px] flex-col items-center px-[clamp(20px,5vw,80px)] pb-[clamp(5rem,9vh,8rem)] pt-[clamp(10rem,18vh,15rem)] max-[768px]:px-5 max-[768px]:pb-16 max-[768px]:pt-[clamp(8rem,17vh,11rem)]">
        ...
        <motion.p
          ...
          className="mb-0 mt-[clamp(18px,2vh,28px)] w-[clamp(300px,38.13vw,650px)] max-w-full text-center font-['Comfortaa',sans-serif] text-[clamp(16px,1.53vw,26px)] font-medium leading-[1.25] text-[#442F0E] whitespace-pre-line max-[768px]:w-full max-[768px]:max-w-[550px] max-[768px]:text-[clamp(16px,4vw,18px)]"
        >
          {descriptionText}
        </motion.p>
      </div>
